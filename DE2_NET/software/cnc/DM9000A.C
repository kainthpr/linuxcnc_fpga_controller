...
//  iow(0x0A, 0); /* RTFCR REG.0AH (if necessary) RX/TX Flow Control Register enable TXPEN, BKPM (TX_Half), FLCE (RX) */
...
//  iow(ETXCSR, 0x80); /* Early Transmit 12.5% */
...
//    __builtin_stwio ((void*)DM9000A_BASE, (data_ptr[i+1]<<8)|data_ptr[i]);
//    IOWR(DM9000A_BASE, IO_data, 0);

  }

  /* issue TX polling command activated */
  iow(TCR , TX_REQUEST);  /* TXCR Bit [0] TXREQ auto clear after TX completed */
...
//    usleep(STD_DELAY);
    ...
    {
//    	outport(SEG7_DISPLAY_BASE,(RxStatus <<16 | rx_len[0]));
    	GoodPacket = TRUE;
...
//      printf("\nError\n");
  ...
  else if((rx_READY != 1) & (rx_READY != 0)) /* status check first byte: rx_READY Bit[1:0] must be "00"b or "01"b */
  {
    /* software-RESET NIC */
    iow(NCR, 0x03);   /* NCR REG. 00 RST Bit [0] = 1 reset on, and LBK Bit [2:1] = 01b MAC loopback on */
    usleep(20);       /* wait > 10us for a software-RESET ok */
